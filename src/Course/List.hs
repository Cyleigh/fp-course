...
filter _ Nil = Nil
filter f (a :. b) = 
  let rm = filter f b in 
  if (f a) then (a :. rm) 
    else rm
{-
filter p list = foldRight (\h t -> if p h then h :. t else t) Nil list
-}
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight (\opta optlista -> bindOptional (\a  -> mapOptional (\lista -> a :. lista) optlista) opta) (Full Nil)
-- use bindOptional and mapOptional
...
find ::
  ...
  -> Optional a
find f Nil = Empty
find f (h :. tl) = if (f h) then Full h else find f tl
...
reverse ::
  List a
  -> List a
reverse = foldLeft (\r x -> x :. r) Nil

-- (++) reverse tl a -- is not tail recursive; hence is not cool
