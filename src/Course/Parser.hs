...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = p >>= (\x -> list p >>= (\tl -> valueParser(x :. tl)) ) 
-- lift2 (:.) <*> list
-- \p -> lift2 (:.) p (list p)
-- p and then (call it x)
-- list p and then (call it tl)
-- always produce x :. tl
...
sequenceParser (h :. tl) = h >>= \x 
                           -> sequenceParser tl >>= \y 
                           -> valueParser (x :. y)
-- (h :. t) => lift2 (:.) h (sequence t)
-- seqParser = sequence (from Applicative)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper >>= \f -> list lower >>= \tl -> valueParser (f :. tl)
...
surnameParser ::
  Parser Chars
surnameParser = upper >>= \f -> thisMany 5 lower >>= \tl -> list lower >>= \fl -> valueParser (f :. tl ++ fl)
-- (\f tl fl -> f :. tl +++ fl) <$>
-- upper <*>
-- thisMany 5 lower <*>
-- list lower
-- NOTE: So the above works in applicative because we don't use the 'call it' values in the next calls; hence we don't need to strengthen to monad
...
phoneParser = digit >>= \d -> phoneBodyParser >>= \pb -> is '#' >>> valueParser (d :. pb)
-- This can be written in applicative             
...
personParser ::
  Parser Person
personParser = (\ag fn sn issmkr ph -> Person ag fn sn issmkr ph) <$>
                ageParser <* spaces1 
...
{-
 ageParser >>= \ag ->
               ...
               spaces1 >>> 
               phoneParser  >>= \ph ->
               valueParser (Person ag fn sn issmkr ph)
               -}
